...
- Antes, había que arreglarse únicamente con el formulario para hacer las peticiones. Esto con el tiempo cambió, con la idea de que las páginas sean SPA. Un programador de Microsoft quería hacer una SPA para la bandeja de entrada de Outlook, él quería que no se recargue la página cuando llegaba un nuevo mail. Que simplemente se actualice la página con el nuevo mail, pero sin recargarse todo desde 0. 
...
0 - state 'UNSENT' (se creó el client)
1 - state 'OPENED' (se llamó a la función open())
2 - state 'HEADERS_RECEIVED' (se llamó a la función send() y se recibieron los headers)
3 - state 'LOADING' (se está descargando la data)
4 - state 'DONE' (se completó la operación)
