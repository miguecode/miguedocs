...
```javascript
fecha.getDate();        // 9         -> Día del mes (1 a 31)
fecha.getDay();         // 3         -> Día de la semana (0 = domingo, 6 = sábado)
fecha.getMonth();       // 3         -> Mes (0 = enero, 11 = diciembre)
fecha.getFullYear();    // 2025      -> Año completo
fecha.getHours();       // 15        -> Hora (0-23)
fecha.getMinutes();     // 30        -> Minutos
fecha.getSeconds();     // 0         -> Segundos
fecha.getMilliseconds();// 0         -> Milisegundos
fecha.getTime();        // 1744218600000 -> Milisegundos desde el Epoch
```
...
```javascript
const ahoraMiliseundos = Date.now(); // Muy útil para timestamps rápidos
```
