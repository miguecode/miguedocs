...
```php
$archivo = fopen("archivo.txt", "r"); // El primer parámetro es la dirección del archivo (local o externo), y el segundo, el modo de abrir el archivo. Devuelve un int. Si hubo error, devuelve 'false'.
  ```

```php
fclose($archivo); // Cerramos el archivo que estaba abierto. Si tuvo éxito retorna TRUE, y sino, FALSE.
```
