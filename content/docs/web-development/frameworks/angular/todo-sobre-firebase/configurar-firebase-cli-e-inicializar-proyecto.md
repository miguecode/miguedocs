...
description: "Para usar Firebase, lo primero que hay que hacer siempre es instalar el Firebase CLI en nuestro sistema. Lo más cómodo es hacerlo a nivel global. Para eso, usam..."
---
...
- Cuando terminamos la inicialización, Firebase nos crea archivos como:

1. .firebaserc - Este archivo define el ID del proyecto Firebase al que estamos conectados. También puede guardar configuraciones por entorno (dev, prov, etc).

2. firebase.json - Este es el archivo central de la configuración de Firebase. Define qué servicios vamos a usar, y cómo se comportan. Si quisieramos sacar algún servicio, simplemente lo borramos de aca.

3. Archivos de reglas o de indexación. Antes, estos archivos aparecían solamente en la consola web de Firebase. Ahora, aparecen acá. Sirven para definir los permisos y el comportamiento de algunos servicios como Firestore o Storage.

## ¿Y ahora?

- Esto no termina acá. Lo próximo que podemos hacer es configurar el servicio de Hosting, porque es el más sencillo y a la vez el más esencial, para que podamos ver que efectivamente podemos ver nuestra app alojada. Pero para los demás servicios, todavía faltan pasos: instalar firebase y @angular/fire. Esto lo vamos a ver en próximos apuntes