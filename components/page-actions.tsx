...
export function SpanishMarkdownCopyButton({
  markdownUrl,
  className,
  ...props
  ...
  return (
    ...
    >
      {checked ? <Check /> : <Copy />}
      {props.children ?? 'Copiar Markdown'}
    </button>
  );
}
...
export function SpanishViewOptionsPopover({
  markdownUrl,
  githubUrl,
  className,
  ...props
  ...
  const items = useMemo(() => {
    const q = `Leí ${typeof window === 'undefined' ? pathname : new URL(pathname, window.location.origin)}, quiero hacer preguntas al respecto.`;
    const list: PopoverItem[] = [];
